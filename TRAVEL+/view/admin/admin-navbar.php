<div class="topnav">
    <h1>TRAVEL+ GH</h1>
    <nav class="bar" id="mainNav">
        <div class="account-container">
            <button class="account" id="accountBtn">
                <span class="material-symbols-outlined">account_circle</span>
            </button>
            
            <div class="account-dropdown" id="accountDropdown">
                <a href="/Web_Warriors_Travelsite/TRAVEL+/view/admin/admin-dashboard.php">My Dashboard</a>
                <a href="/Web_Warriors_Travelsite/TRAVEL+/view/wishlist.php">My Wishlist</a>
                <a href="/Web_Warriors_Travelsite/TRAVEL+/view/admin/user-management.php">User Management</a>
                <a href="/Web_Warriors_Travelsite/TRAVEL+/view/admin/location-management.php">Location Management</a>
                <a href="/Web_Warriors_Travelsite/TRAVEL+/view/logout.php">Logout</a>
            </div>
        </div>
        
        <!-- Navigation Links -->
        <a href="/Web_Warriors_Travelsite/TRAVEL+/view/contact-us.php">Contact Us</a>
        <a href="/Web_Warriors_Travelsite/TRAVEL+/view/about.php">About The Project</a>
        <a href="/Web_Warriors_Travelsite/TRAVEL+/view/all-blogs.php">Blogs</a>
        <a href="/Web_Warriors_Travelsite/TRAVEL+/view/all-locations.php">Locations</a>
    </nav>

    <div>
        <button class="view-more" id="viewMoreBtn">
            <span class="material-symbols-outlined">menu</span>
        </button>
    </div>
</div>

<script src="/Web_Warriors_Travelsite/TRAVEL+/assets/js/admin-navbar.js"></script>